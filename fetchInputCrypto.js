...
var getLatestValPerTokenInUSD = function () {
    return new Promisefunction (resolve) {
        ...
        var kadenaOutput ={ "token": "kadena", "amount": 0, "timestamp": 0 };
        ...
        }else if (jsonFromLine.token === 'kadena') {
            if(jsonFromLine.timestamp > kadenaOutputArr.timestamp){
                kadenaOutputArr.amount=jsonFromLine.amount;
                kadenaOutputArr.timestamp = jsonFromLine.timestamp;
        ...
        lineReader.on('close', function (line) {
            ...
            cryptoCompare.then(function (result) {
                ...
                kadenaOutputArr.amount = kadenaOutput.amount*usdValues.ETH.USD;
                ltcOutputArr.amount = ltcOutput.amount*usdValues.ETH.USD;


                output.push(ethOutputArr);
                output.push(btcOutputArr);
                output.push(xrpOutputArr);
                output.push(kadenaOutputArr);
    ...
    return new Promise(function (resolve) {
        ...
        lineReader.on('line', function (line) {
            ...
            if (jsonFromLine.token === 'ETH') {
                ...
                if (args.date === dateFromCSV) {
                    kadenaOutputArr.push({ "token": jsonFromLine.token, "amount": jsonFromLine.amount * usdValues.ETH.USD });
                }
            }
